      ...
      <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="p-5 overflow-y-auto space-y-5 text-xs text-slate-200 flex-1">
            ...
            {/* EV Tow Vehicle Setup */}
            <div className="space-y-3 bg-cyan-950/20 p-3.5 rounded-xl border border-cyan-500/30">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <i className="fa-solid fa-bolt text-cyan-400 text-sm"></i>
                  <h4 className="font-bold text-cyan-300 uppercase tracking-wider text-[11px]">
                    Electric Tow Vehicle (EV Towing Mode)
                  </h4>
                </div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <span className="text-[11px] text-slate-300 font-medium">Enable EV Mode</span>
                  <input
                    type="checkbox"
                    checked={Boolean(localProfile.isEvTowVehicle)}
                    onChange={(e) => setLocalProfile({ ...localProfile, isEvTowVehicle: e.target.checked })}
                    className="rounded bg-slate-900 border-slate-700 text-cyan-500 focus:ring-0"
                  />
                </label>
              </div>

              {localProfile.isEvTowVehicle && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                  <div>
                    <label className="block text-slate-400 mb-1">EV Truck / SUV Model</label>
                    <select
                      value={localProfile.evModel || "Ford F-150 Lightning"}
                      onChange={(e) => setLocalProfile({ ...localProfile, evModel: e.target.value })}
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-xs text-slate-200 focus:border-cyan-500"
                    >
                      <option value="Ford F-150 Lightning">Ford F-150 Lightning</option>
                      <option value="Rivian R1T / R1S">Rivian R1T / R1S</option>
                      <option value="Tesla Cybertruck">Tesla Cybertruck</option>
                      <option value="Chevy Silverado EV">Chevy Silverado EV</option>
                      <option value="Tesla Model X / Y">Tesla Model X / Y</option>
                      <option value="Hyundai Ioniq 5 / Kia EV6 / EV9">Hyundai / Kia EV</option>
                      <option value="Custom Electric Tow Vehicle">Other EV Model</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-slate-400 mb-1">
                      Realistic Towing Range (Miles) <span className="text-cyan-400 font-bold">*Towing</span>
                    </label>
                    <input
                      type="number"
                      min="50"
                      max="400"
                      value={localProfile.evTowingRangeMiles ?? 140}
                      onChange={(e) => setLocalProfile({ ...localProfile, evTowingRangeMiles: parseInt(e.target.value, 10) || 140 })}
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-xs text-cyan-300 font-bold focus:border-cyan-500"
                      placeholder="e.g. 140"
                    />
                    <span className="text-[10px] text-slate-400 block mt-0.5">Towing uses ~40–50% more kWh</span>
                  </div>

                  <div>
                    <label className="block text-slate-400 mb-1">Charging Connector Preference</label>
                    <select
                      value={localProfile.evPlugType || "NACS (Tesla Supercharger)"}
                      onChange={(e) => setLocalProfile({ ...localProfile, evPlugType: e.target.value })}
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-xs text-slate-200 focus:border-cyan-500"
                    >
                      <option value="NACS (Tesla Supercharger)">NACS (Tesla Supercharger)</option>
                      <option value="CCS1 (Electrify America / EVgo)">CCS1 (Electrify America / EVgo)</option>
                      <option value="Universal (NACS + CCS Adapter)">Universal (NACS + CCS Adapter)</option>
                    </select>
                  </div>
                </div>
              )}
