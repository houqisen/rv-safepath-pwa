# 🚐 RV SafePath — Smart RV Copilot & Safe Route Planner
...
> **RV SafePath** is a Progressive Web App (PWA) designed specifically for RVers, camper vans, and travel trailers. It pairs your rig's exact dimensions (*height clearance, combined length, gross weight, and towing MPG*) with Google Maps and Gemini AI to provide low-clearance safe routing, instant facility discovery, live towing weather hazard alerts, and automated multi-day trip itineraries.
...
### ⛅ 5. Live Towing Weather & Hazard Radar
...
### ✅ 6. Interactive Departure & Setup Checklists
...
- **AI Engine:** Google Gemini API (`gemini-flash-lite-latest`, `gemini-3.5-flash`, `gemini-flash-latest`)
...
- [Google Cloud API Key](https://console.cloud.google.com/) with the following APIs enabled:
  - Maps JavaScript API
  - Places API (New)
  - Directions API
  - Geocoding API
...
RV SafePath is fully installable as a standalone app:
...
- Your RV vehicle specs and trip itineraries are saved locally in your browser via `localStorage`.
- Gemini API keys stored in settings remain strictly on the client side and are never transmitted to third-party tracking servers.
